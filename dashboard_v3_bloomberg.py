...
# Suppress all SDK/HTTP logging and stdout spam
# ─────────────────────────────────────────────

logging.disable(logging.CRITICAL)
os.environ.setdefault("FYERS_LOG_LEVEL", "NONE")

@contextlib.contextmanager
def _silent():
    """Redirect stdout/stderr to /dev/null for noisy SDK calls."""
...
# ─────────────────────────────────────────────
# CONFIG  (no hardcoded secrets — use secrets.toml or env vars)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# BLOOMBERG DARK THEME  (defined early so auth UI can use BBG colours)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# AUTHENTICATION GATE
# Reads DASH_USER / DASH_PASSWORD from secrets.toml
# ─────────────────────────────────────────────

def _auth_gate():
    valid_user = get_secret("DASH_USER")
    valid_pass = get_secret("DASH_PASSWORD")

    if not valid_user or not valid_pass:
        st.error("⚠️  DASH_USER and DASH_PASSWORD are not configured in secrets.toml")
        st.stop()
    ...
    <style>
        html, body, .stApp {{ background: {BBG['bg']} !important; }}
        header[data-testid="stHeader"] {{ display: none !important; }}
        .block-container {{ padding: 0 !important; }}
    </style>
    <div style="
        display:flex; align-items:center; justify-content:center;
        min-height:100vh; background:{BBG['bg']};
    ">
        <div style="
            background:{BBG['card']}; border:1px solid {BBG['card_bdr']};
            border-top:3px solid {BBG['orange']}; padding:44px 52px;
            min-width:380px; font-family:'IBM Plex Mono',monospace;
        ">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:28px;">
                <div style="background:{BBG['orange']};color:#000;font-weight:700;
                            font-size:13px;padding:4px 10px;letter-spacing:1px;">BBG</div>
                <div>
                    <div style="color:{BBG['text']};font-weight:600;font-size:13px;
                                letter-spacing:1px;text-transform:uppercase;">
                        NFO / BFO Terminal
                    </div>
                    <div style="color:{BBG['text3']};font-size:9px;letter-spacing:1px;
    ...
    with st.form("login_form", clear_on_submit=False):
        st.markdown(
            ...
            f"text-transform:uppercase;color:{BBG['text3']};margin-bottom:4px;"
            f"font-family:IBM Plex Mono'>Username</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"<div style='font-size:9px;font-weight:700;letter-spacing:2px;"
            f"text-transform:uppercase;color:{BBG['text3']};margin-top:12px;margin-bottom:4px;"
            f"font-family:IBM Plex Mono'>Password</div>",
            unsafe_allow_html=True,
        )
...
_auth_gate()  # ← blocks everything below until logged in
...
def get_shared_token():
    """Generates / loads token once — shared across all browser sessions."""
    try:
...
.bbg-logo {{ display: flex; align-items: center; gap: 12px; }}
.bbg-logo-box {{
    background: var(--orange); color: #000 !important;
    font-family: 'IBM Plex Mono', monospace !important; font-weight: 700;
    font-size: 13px; padding: 4px 10px; letter-spacing: 1px;
}}
.bbg-title {{
    font-family: 'IBM Plex Mono', monospace !important; font-size: 13px;
    font-weight: 600; color: var(--text) !important; letter-spacing: 1px;
    text-transform: uppercase;
}}
.bbg-subtitle {{
    font-size: 10px; color: var(--text3) !important;
    font-family: 'IBM Plex Mono', monospace !important;
    letter-spacing: 0.5px; text-transform: uppercase;
}}
.bbg-pills {{ display: flex; gap: 12px; align-items: center; }}
.bbg-pill {{
    font-family: 'IBM Plex Mono', monospace !important; font-size: 10px;
    font-weight: 600; letter-spacing: 1px; padding: 3px 10px;
    border: 1px solid; text-transform: uppercase;
}}
.pill-live {{
    color: var(--pe) !important; border-color: var(--pe) !important;
    background: rgba(0,204,102,0.08); animation: blink 1.8s infinite;
}}
.pill-time {{
    color: var(--yellow) !important; border-color: #3a3020 !important;
...
.metric-card {{
    ...
    border-top: 2px solid; padding: 14px 16px 12px;
...
.metric-value {{
    font-family: 'IBM Plex Mono', monospace !important; font-size: 26px;
...
.metric-sub {{
    font-size: 10px; color: var(--text3) !important;
    font-family: 'IBM Plex Mono', monospace !important; letter-spacing: 0.3px;
}}
.metric-badge {{ position: absolute; top: 10px; right: 12px; font-size: 16px; opacity: 0.25; }}
.metric-divider {{ width: 100%; height: 1px; background: var(--divider); margin: 8px 0; }}
...
.stTextInput input,
.stNumberInput input {{
    background: var(--card) !important; border-color: #2c2c2c !important;
...
.stDateInput input {{
    background: var(--card) !important; border-color: #2c2c2c !important;
    color: var(--text) !important; border-radius: 2px !important; font-size: 12px !important;
}}
...
.stButton > button[data-testid="baseButton-primary"] {{
    ...
    font-family: 'IBM Plex Mono', monospace !important; transition: all 0.15s !important;
}}
.stButton > button[kind="primary"]:hover {{ background: #cc5200 !important; transform: none !important; }}
.stButton > button:not([kind="primary"]) {{
    background: #1a1a1a !important; color: var(--orange) !important;
...
.stButton > button:not([kind="primary"]):hover {{ border-color: var(--orange) !important; }}

/* Tabs */
.stTabs [data-baseweb="tab-list"] {{
    background: var(--bg2) !important; border-bottom: 1px solid #222 !important; gap: 0 !important;
}}
.stTabs [data-baseweb="tab"] {{
    background: transparent !important; color: var(--text3) !important;
    border-bottom: 2px solid transparent !important; border-radius: 0 !important;
    padding: 8px 20px !important; font-size: 11px !important; letter-spacing: 1px !important;
    text-transform: uppercase !important; font-family: 'IBM Plex Mono', monospace !important;
    font-weight: 500 !important; transition: all 0.15s !important;
}}
.stTabs [data-baseweb="tab"]:hover {{ color: var(--text) !important; background: #161616 !important; }}
.stTabs [aria-selected="true"] {{
    color: var(--orange) !important; border-bottom: 2px solid var(--orange) !important;
    background: #141414 !important;
}}
.stTabs [data-baseweb="tab-panel"] {{ background: var(--bg) !important; padding-top: 16px !important; }}

/* Misc */
hr {{ border-color: #1e1e1e !important; margin: 10px 0 !important; }}
.stExpander {{ background: var(--card) !important; border: 1px solid var(--card-bdr) !important; border-radius: 2px !important; }}
...
.leg-sep {{ padding-top: 28px; font-size: 11px; color: #2a2a2a; text-align: center; font-family: 'IBM Plex Mono', monospace; }}
div[data-testid="stMainBlockContainer"] > div > div > div:nth-child(1) button {{
    ...
    z-index: 9999 !important; background: rgba(255,102,0,0.1) !important;
    border: 1px solid rgba(255,102,0,0.35) !important; color: var(--orange) !important;
    border-radius: 2px !important; padding: 3px 12px !important; font-size: 10px !important;
...
if st.button("↺ Refresh Token", key="refresh_token_nav",
             help="Clear cached token and re-authenticate"):
...
def fetch_live_data(sym_sx_ce, sym_sx_pe, sym_nf_ce, sym_nf_pe,
    ...
    if any(df.empty for df in [df_sx_ce, df_sx_pe, df_nf_ce, df_nf_pe]):
        st.warning(f"⚠️ One or more symbols returned no data.\n"
                   f"Built: `{sym_sx_ce}` | `{sym_sx_pe}` | `{sym_nf_ce}` | `{sym_nf_pe}`")
        return pd.DataFrame()
...
def chart_layout(fig, title, height=380):
    fig.update_layout(
        title=dict(text=title,
                   font=dict(size=11, color=BBG["text3"], family="IBM Plex Mono"), x=0),
        height=height,
        ...
        yaxis=dict(gridcolor=BBG["grid"], title="Value (₹)", tickfont=dict(size=10),
                   showspikes=True, spikemode="across",
                   spikecolor=BBG["text3"], spikethickness=1, spikedash="dot",
                   tickcolor=BBG["text3"], linecolor="#2a2a2a"),
...
def make_hlines(fig, series, hi_col, lo_col, row=None, col=None):
    ...
    fig.add_hline(y=h, line_dash="dot",  line_color=hi_col, line_width=1,
                  ...
                  annotation_font=dict(color=hi_col, size=9, family="IBM Plex Mono"), **kw)
    fig.add_hline(y=l, line_dash="dot",  line_color=lo_col, line_width=1,
                  annotation_text=f"L: {l:.0f}", annotation_position="right",
                  annotation_font=dict(color=lo_col, size=9, family="IBM Plex Mono"), **kw)
...
with tab2:
    ...
    if not df_custom.empty:
        ...
        fig2.add_trace(go.Scatter(
            ...
            fill="tozeroy", fillcolor="rgba(153,102,255,0.07)",
...
with tab1:
    ...
    legs_row[0].markdown(
        ...
        f"font-family:IBM Plex Mono'>LEG 1</div>", unsafe_allow_html=True)

    with legs_row[1]:  sensex_exchange   = st.selectbox("Exchange",
                            ["BSE","NSE"], index=0, key="sx_exch")
    ...
    legs_row[7].markdown("<div class='leg-sep' style='padding-top:28px'>│</div>",
                         unsafe_allow_html=True)
    legs_row[8].markdown(
        f"<div style='padding-top:28px;font-size:10px;font-weight:700;"
        f"letter-spacing:1.5px;text-transform:uppercase;color:{BBG['blue']};"
        f"font-family:IBM Plex Mono'>LEG 2</div>", unsafe_allow_html=True)

    with legs_row[9]:  nifty_exchange    = st.selectbox("Exchange",
                            ["NSE","BSE"], index=0, key="nf_exch")
    ...
    if df.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=BBG["ce"], width=1.5),
            hovertemplate="%{x|%H:%M}<br>CE Spread: %{y:.2f}<extra></extra>"), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df.index, y=df["pe_spread"], name="PE Spread",
            line=dict(color=BBG["pe"], width=1.5),
            hovertemplate="%{x|%H:%M}<br>PE Spread: %{y:.2f}<extra></extra>"), row=1, col=1)
        make_hlines(fig, df["ce_spread"], BBG["ce"], BBG["pe"], row=1, col=1)

        if show_diff:
            fig.add_trace(go.Scatter(
                x=df.index, y=df["diff"], name="4-Leg Total",
                line=dict(color=BBG["diff"], width=1.5),
                fill="tozeroy", fillcolor="rgba(255,170,0,0.07)",
                hovertemplate="%{x|%H:%M}<br>4-Leg Total: %{y:.2f}<extra></extra>"), row=2, col=1)
        ...
        fig.update_layout(
            height=total_h,
            plot_bgcolor=BBG["plot_bg"], paper_bgcolor=BBG["plot_bg"],
            font=dict(color=BBG["text2"], family="IBM Plex Mono"),
            hovermode="x unified",
        ...
        for i in range(1, n_rows + 1):
            fig.update_xaxes(gridcolor=BBG["grid"], tickfont=dict(size=10),
                             showspikes=True, spikemode="across",
                             spikecolor=BBG["text3"], spikethickness=1, spikedash="dot",
                             tickcolor=BBG["text3"], linecolor="#2a2a2a", row=i, col=1)
...
with tab3:
    ...
    if skew_btn:
        ...
        for K in strikes:
            ...
            iv_ce = max(atm_iv + 0.3 * moneyness**2 - 0.05 * moneyness, 0.01)
            iv_pe = max(atm_iv + 0.3 * moneyness**2 + 0.08 * moneyness, 0.01)
        ...
        fig_skew.add_trace(go.Scatter(
            x=df_skew["strike"], y=df_skew["iv_ce"], name="CE IV",
            line=dict(color=BBG["ce"], width=2), mode="lines+markers",
            marker=dict(size=5, color=BBG["ce"]),
            hovertemplate="Strike: %{x:.0f}<br>CE IV: %{y:.2f}%<extra></extra>"))
        fig_skew.add_trace(go.Scatter(
            x=df_skew["strike"], y=df_skew["iv_pe"], name="PE IV",
            line=dict(color=BBG["pe"], width=2), mode="lines+markers",
        ...
        fig_price.add_trace(go.Bar(
            x=df_skew["strike"], y=df_skew["price_ce"], name="CE Price",
            marker_color=BBG["ce"], opacity=0.8,
            hovertemplate="Strike: %{x:.0f}<br>CE: ₹%{y:.2f}<extra></extra>"), row=1, col=1)
        fig_price.add_trace(go.Bar(
            x=df_skew["strike"], y=df_skew["price_pe"], name="PE Price",
            marker_color=BBG["pe"], opacity=0.8,
            hovertemplate="Strike: %{x:.0f}<br>PE: ₹%{y:.2f}<extra></extra>"), row=1, col=2)
    ...
    if pay_btn:
        ...
        pnl = ((intrinsic - pay_prem) if pay_type == "Buy" else (pay_prem - intrinsic)) * pay_lots
        ...
        fig_pay.add_trace(go.Scatter(
            x=S_arr[pos_mask], y=pnl[pos_mask], fill="tozeroy",
            fillcolor="rgba(0,204,102,0.15)", line=dict(color=BBG["pe"], width=0),
            name="Profit", hoverinfo="skip"))
        fig_pay.add_trace(go.Scatter(
            x=S_arr[neg_mask], y=pnl[neg_mask], fill="tozeroy",
            fillcolor="rgba(255,102,51,0.15)", line=dict(color=BBG["ce"], width=0),
            name="Loss", hoverinfo="skip"))
        fig_pay.add_trace(go.Scatter(
            x=S_arr, y=pnl, name=f"{pay_type} {pay_opt}",
        ...
        fig_pay.add_vline(x=pay_spot,   line_dash="dot",  line_color=BBG["blue"],
                           ...
                           annotation_font_color=BBG["blue"], annotation_font_size=9)
        fig_pay.add_vline(x=pay_strike, line_dash="dash", line_color=BBG["text3"],
                           annotation_text="Strike",
                           annotation_font_color=BBG["text3"], annotation_font_size=9)
        fig_pay.add_hline(y=0, line_dash="solid", line_color="#333", line_width=1)
        chart_layout(fig_pay,
                     f"PAYOFF AT EXPIRY — {pay_type.upper()} {pay_opt} | "
